 ...
 */
int mz_is_chain(info_mz *info, char *buf, size_mz *p)
{
size_mz j = *p;
if (buf[j] == '|' && buf[j + 1] == '|')
{
buf[j] = 0;
j++;
info->cmd_buf_mzype = CMD_OR;
}
else if (buf[j] == '&' && buf[j + 1] == '&')
{
buf[j] = 0;
j++;
info->cmd_buf_mzype = CMD_AND;
}
else if (buf[j] == ';') /* found end of this command */
{
buf[j] = 0; /* replace semicolon with null */
info->cmd_buf_mzype = CMD_CHAIN;
}
else
return (0);
*p = j;
 ...
 * Return: void valuee
 */
void mz_check_chain(info_mz *info, char *buf, size_mz *p, size_mz i, size_mz len)
{
size_mz j = *p;
if (info->cmd_buf_mzype == CMD_AND)
{
if (info->status)
{
buf[i] = 0;
 ...
 */
int mz_replace_alias(info_mz *info)
{
int i;
lismz_mz *node;
char *p;
 ...
 */
int mz_replace_vars(info_mz *info)
{
int i = 0;
lismz_mz *node;
